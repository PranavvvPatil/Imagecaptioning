        ...
        <div className="flex justify-between items-center">
          ...
          <div className="hidden md:flex items-center space-x-8">
            {["Privacy Policy", "Contact Us"].map((item) => {
              const path = item === "Contact Us" ? "/reach-out" : `/${item.toLowerCase().replace(" ", "-")}`;
              return (
                <Link
                  key={item}
                  to={path}
                ...
                >
                  ...
                  <span className={`absolute bottom-0 left-0 w-full h-0.5 bg-gradient-to-r from-violet-400 to-purple-400 transform origin-left transition-all duration-300 ${
                    isActivePath(path) ? 'scale-x-100' : 'scale-x-0 group-hover:scale-x-100'
                  }`} />
                </Link>
              );
            })}
